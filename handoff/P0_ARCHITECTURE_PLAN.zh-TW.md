...
- 保留 root `DatabaseConnect.py` 作為相容 wrapper，但文件要標示它不是主要開發入口。
- `records/range` 目前是播放效能最大阻塞點之一；P0 只記錄邊界，P1 再做查詢策略最佳化。
...
- `esri_ocean_basemap` 是 Esri basemap provider id，語意上不是本專案 ocean domain；前端保留 `esri_ocean` legacy alias 只為了相容舊狀態。
...
- Playback state machine：播放、預熱、range cache、render artifact cache 應收斂成狀態表與 transition，不繼續散在 click handler 與 cache service。
