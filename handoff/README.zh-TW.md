...
不需要交付前端 UI、GFW/NASA、EEZ、Leaflet 或 WebGL 程式。
...
- `config/adapter.example.json` 是模板，不放真密碼。
- `config/adapter.local.json` 與 `config/ais_collector.local.json` 是本機 secrets/override，已被 `.gitignore` 忽略。
