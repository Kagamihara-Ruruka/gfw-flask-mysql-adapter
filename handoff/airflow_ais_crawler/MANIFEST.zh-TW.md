...
- `AisIngestService.py`：AISStream websocket 長駐收集器、upsert、snapshot、heartbeat 主體。
- `AisStreamProvider.py`：AISStream websocket 連線、subscription message、AIS message normalize。
- `AisLiveService.py`：AIS SQL read model 與 MySQL connection helper。
- `DatabaseConnect.py`：config loader、MySQL connection、identifier validation。
- `database/registry.py`：backend registry，讓 config loader 能辨識 SQL backend。
- `core.py`：CLI 入口，使用 `ingest-ais` command 啟動 crawler。

## 必要設定

- `config/adapter.ais_collector.example.json`：crawler 專用最小 adapter config。
