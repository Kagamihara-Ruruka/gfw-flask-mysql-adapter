...
from DatabaseConnect import validate_identifier
from DeveloperConfigService import load_layer_mappings, load_router_manifest, normalize_imported_layers
    ...
    runtime.update(
        {
            ...
            "__runtime_source": "manual_mapping_contract",
            "__runtime_mapping_id": mapping.get("mapping_id"),
            "__runtime_config_path": mapping.get("config_path"),
        }
    )
    return runtime


def active_manual_mapping_for_layer(layer_id: str) -> dict[str, Any] | None:
    layer = str(layer_id or "").strip().lower()
    manifest = load_router_manifest()
...
def resolve_runtime_dataset(
    ...
    mapping = active_manual_mapping_for_layer(layer_id)
    if not mapping:
        return base_dataset, {
            "layer_id": layer_id,
            "source": "legacy_dataset_contract",
            "mapping_id": None,
            "config_path": None,
        }
    ...
    return runtime_dataset, {
        "layer_id": layer_id,
        "source": "manual_mapping_contract",
