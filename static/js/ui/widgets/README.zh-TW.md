...
每個 Widget 子類別負責自己的空白範本與視覺語意。卡片內是摘要視圖；高度一列的圖表使用輕量 SVG，高度二列或展開後的完整圖表使用 Plotly。Widget 只呼叫注入的 Application service；Application DataSource 必須先讀 `DataFrameStore`，只有明確允許補資料的圖表才能透過 `FrameDemandService` 提出 widget lane demand，不能直接呼叫 source API。表格與生命週期事件檢視器是嚴格唯讀工具，cache miss 也不能補查。資料查詢合約與視覺範本分離，未定義資料來源時只呈現明確的等待綁定狀態。
