...
from common_adapter.config.paths import canonical_config_ref, resolve_repo_path, router_manifest_path

ROOT = Path(__file__).resolve().parents[2]


def resolve_config_path(value: str | Path, *, base_path: Path | None = None) -> Path:
    canonical = resolve_repo_path(canonical_config_ref(value))
    if canonical.exists():
        return canonical
    path = Path(value)
    if path.is_absolute():
...
def _router_manifest_active_refs() -> list[str]:
    ...
    if not isinstance(active, list):
        return []
    return [canonical_config_ref(item) for item in active if str(item).strip()]
