...
它不應該直接塞 MySQL、PostGIS、AISStream API 等具體協定細節，也不持有另一份啟用路由集合、dataset 清單或啟動遷移規則。開發者頁若顯示它，只能把它視為一般 JSON runtime 入口；它不是 source router，也不與 `DATABASE`、`WEBSOCKET`、`SPATIAL`、`ENDPOINT` 並列。
...
- `domain_mask` 的版本化 topology cache；此 cache 只分類 exact EEZ 補集合的陸地／公海語意，視覺海岸線仍由目前 z/x/y 的 EEZ LOD tile 決定
...
- websocket endpoint
- API key fingerprint
- position/static message types
- collector handoff path
- read model 的 `connection_ref`、database、table 與欄位名稱

這份 route config 描述唯一的端到端能力：`AISStream delta source -> collector merge -> MySQL read model -> dashboard read`。它只以 `connection_ref` 引用 DATABASE Registry 的連線，不內嵌密碼，也沒有隱性預設 MySQL。獨立 collector 啟動所需的 raw key 與具體 sink 連線只會被解析到 gitignored 的 `config/runtime/ais_collector.local.json` handoff；Dashboard 不讀 handoff，也不直接連 AISStream。

位置與靜態資料分別保存來源事件時間，`received_at` 只表示 collector 收到訊息的時間。collector 依 MMSI 合併 delta，不得用缺少欄位的訊息把 read model 既有值寫成 null。AISHub 不再是備援路徑；Kafka 等 broker 只列為未來 upstream 演進方向。
...
`DATABASE`、`WEBSOCKET`、`SPATIAL`、`ENDPOINT` 是目前已實作 probe adapter 的內建 source kind，不是資料源類型的永久上限。未來新增其他 source kind 時，應先新增 `config/sources/<kind>/` 與對應 JSON；若尚未有 probe adapter，控制頁可以列出 source config，但不會把它注入下游 Schema、Mapping 或 Layer Contract。Pipeline Iceberg 目前是 `DATABASE` source 上的 HTTP endpoint adapter，不等同於尚未實作的直接 Spark／Hive backend。
...
Browser Profile 不屬於 Config 控制面。它由前端 DI 建立的 `BrowserProfileStoreCore` 使用 `localStorage` 保存裝置與視覺偏好，例如地圖互動、底圖、paint/alpha、renderer 與 AIS 顯示策略。資料源、圖層啟用、日期、Mapping、Query Policy、cache/watermark 與 playback state 不得進入 Browser Profile。瀏覽器禁止 storage 時只退回目前 session，不改變 Runtime 真相。
...
Mapping Controller 是前後端共同的控制器，前端位置在開發者頁的「路由狀態機」與「資料圖層導入」之間。它不是 router，也不屬於 `DATABASE`、`WEBSOCKET`、`SPATIAL` 或 `ENDPOINT` 分類。
...
- 哪些欄位是時間、經度、緯度、識別欄
- 哪些欄位要作為顯示欄、指標欄或分類欄
- sampled-grid 的可用解析度、coverage 邊界與初始 `default_coverage_id`
- source request 欄位、canonical roles 與額外 canonical extension columns
- source status 字彙到 canonical `observed / filled / no_data / unknown` 的值域映射

使用者不需要手動串接 source、schema、mapping、layer contract 與 dashboard。系統會把 mapping 寫入 `config/artifacts/layer_mappings.local.json`，使用者只需要在頁面上勾選是否把已生成的資料圖層導入儀表板。Mapping 產物可以引用 `DATABASE` source 作為上游資料來源，但它本身是服務內部協議。Source Scout 永遠呈現來源實際提供的欄位；Mapping 只能在探測結果之後決定查詢投影與 canonical 語意，不能反向隱藏來源 schema。

對 user-authored Mapping，未選欄位才表示不進查詢投影。對 source-contract/generated Mapping，UI 以「未映射」顯示來源欄位，不以「不查詢」假裝它能編輯唯讀來源合約；控制面必須同時揭露 supported、editable 與 provenance。
...
`sampled_grid.status_semantics` 是資料狀態值域的唯一翻譯表。Scout 可觀測 `contains_filled` 等來源字彙，但 Compiled Mapping 必須在 Canonical 邊界將它們正規化；Renderer 只能消費 canonical status 與 Paint Policy，不得硬編碼任何 provider status。這份語意也屬於 mapping/cache identity，變更後不能重用舊 Frame。

若來源 catalog 宣告互相重疊的 coverage，`default_coverage_id` 只決定初始視角與沒有 BBOX 時的預設查詢區域。實際 viewport 查詢會由 adapter 選擇單一 coverage：優先使用包含視窗中心的最小 coverage，否則使用交疊面積最大的 coverage；不得把巢狀 AOI 聯集後再誤判成較粗 LOD。
